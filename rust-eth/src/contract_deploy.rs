...
use ethers_solc::{ConfigurableArtifacts, Project, ProjectCompileOutput, ProjectPathsConfig}; // Ensure you're using only one `Bytes`
...
async fn main() -> Result<()> {
    ...
    let contract = project
        .find("examples/", contract_name) // Provide both the path and contract name
